...
let popupPos = document.querySelector(".popup")?.getBoundingClientRect().top;

const distanceFromTop = 10;
const distanceTraveledThreshhold = 100;

function getBackgroundColour() {
  const scrollBoxRect: DOMRect = document
    .querySelector(".scrollBox")!
    .getBoundingClientRect();
  const percentage: number = Math.min(
    Math.max(
      ((scrollPos - scrollBoxRect.top) / scrollBoxRect.bottom -
        scrollBoxRect.top) /
  ...
  const color = `linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(${
    255 - 255 * percentage
  },255,255,1) 100%)`;
  const scrollBox: HTMLElement | null = document.querySelector(".scrollBox");
...
<template>
  <!-- 
	...
	-->
  <div class="scrollBox justify-content-center row">
    <div
      class="popup text-center bg-light col-4 rounded border border-dark border-3"
    >
